...
public:
  ...
  double default_active_set_eps_iter_ref;    //"qpOases wrapper early termination tolerance for iterative refinement"// NOLINT
  ...
  double default_qp_smoothing_eps_iter_ref;  //"qpOases wrapper early termination tolerance for iterative refinement" // NOLINT
  ...
  double PathLength;                 //预计的轨迹长度，即路径DP采样的前探距离， 规划的参考线要比PathLength这个长,长度要和目标速度搭配调,速度越大合成的轨迹长度越长
  ...
  double FLAGS_trajectory_time_max_interval;        //"(seconds) Trajectory time interval when publish. The is the max value."
  double FLAGS_trajectory_time_high_density_period; //(seconds) Keep high density in the next this amount of seconds.
